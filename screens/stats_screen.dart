...
class _StatsScreenState extends State<StatsScreen> with SingleTickerProviderStateMixin {
  ...
  bool _recordDialogShown = false; // ✅ Nouveau

  List<Map<String, dynamic>> _playerStats = List.generate(4, (index) => {
    'score': 0,
    'total': 0,
    'best': 0,
    'games': 0,
  ...
  Future<void> _loadStats() async {
    ...
    if (stored.isNotEmpty) {
      setState(() {
        for (int i = 0; i < 4; i++) {
          ...
          if (entries != null && entries.isNotEmpty) {
            ...
            final freq = {5: 0, 10: 0, 15: 0, 25: 0, 50: 0};
            for (var s in scores) {
      ...
      _showNewRecordPopup(); // ✅ Lancement une seule fois
    }
  }

  void _showNewRecordPopup() {
    if (_recordDialogShown) return; // ✅ déjà affiché ? on quitte
    _recordDialogShown = true;

    final topScores = <String, int>{};

    for (int i = 0; i < 4; i++) {
      final score = _playerStats[i]['score'] ?? 0;
      if (score > 0) {
        topScores['J${i + 1}'] = score;
      }
    }

    if (topScores.isEmpty) return;

    final sorted = topScores.entries.toList()
      ..sort((a, b) => b.value.compareTo(a.value));

    final top = sorted.first;
    final second = sorted.length > 1 ? sorted[1] : null;

    if (second != null && top.value == second.value) return;
  ...
  void _resetPlayerStats(int index) {
    ...
    setState(() {
      _playerStats[index] = {
        ...
        'games': 0,
        'group': '-',
        'frequencies': {5: 0, 10: 0, 15: 0, 25: 0, 50: 0},
        'history': <Map<String, dynamic>>[],
      };
    });
    ...
    updatedFreqs[index] = {5: 0, 10: 0, 15: 0, 25: 0, 50: 0};
