...
// Déclaration de la variable partieLancee
bool partieLancee = false; // Variable pour suivre l'état de la partie
...
/*
  // Envoi de données à ESP32-1 (WebSocket)
  Serial1.println("{\"action\": \"update\", \"message\": \"Hello ESP32-1 (WebSocket)\"}");  
  // Envoi de données à ESP32-2 (Firestore)
  Serial2.println("{\"pseudo\": \"Player1\", \"score\": 150}"); 
  // Ajoute un délai pour éviter des envois trop fréquents
  delay(2000);

  // Lecture des données de ESP32-1 (WebSocket)
  if (Serial1.available()) {
    String dataESP32_1 = Serial1.readStringUntil('\n');
    Serial.println("Données reçues de ESP32-1 : " + dataESP32_1);
  }
  // Lecture des données de ESP32-2 (Firestore)
  if (Serial2.available()) {
    String dataESP32_2 = Serial2.readStringUntil('\n');
    Serial.println("Données reçues de ESP32-2 : " + dataESP32_2);
  }
  delay(100);  // Petite pause pour éviter une surcharge des lectures
*/
// ====================
...
/*
  if (Serial.available() > 0) {
    ...
    message.trim(); // Supprime les espaces et les caractères de nouvelle ligne
    handleMessage(message); // Traite le message reçu
  }
*/
 communications();
/*
  if (Serial1.available()) {
    ...
    if (message == "CF1") {
      ...
      startGame();
    } else if (message == "FIN_GAME") {
      // Fin de la partie
      Serial.println("🎮 Commande FIN_GAME reçue !");
      endGame();
    }
  }
*/
